...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the builder username if it exists
    if id builder &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER builder
    fi
    ...
    if [[ "$NB_USER" != "builder" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/builder "/home/$NB_USER"
        fi
        # if workdir is in /home/builder, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/builder/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u builder)" && "$NB_GID" == "$(id -g builder)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^builder:/nayvoj:/" > /tmp/passwd
